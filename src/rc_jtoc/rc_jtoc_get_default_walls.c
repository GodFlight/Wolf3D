...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/18 23:49:21 by sbecker           #+#    #+#             */
...
static int	fill_default_wall_by_pos(t_wall *wall, int **texture,
		t_jnode *n, int id)
{
	t_jnode	*tmp;
	char	*s;
...
static int	get_default_wall_textures(t_wall *wall, t_conf_json *conf,
		t_jnode *n, int id)
{
	t_jnode	*tmp;
	int		**tmp_texture;
	...
	while (tmp)
	{
		...
		if (rc_jtoc_fill_texture_by_index(&tmp_texture, conf, tmp, id))
			return (rc_jtoc_sdl_log_error("FILL WALL TEXTURE BY ID FAILURE",
						id));
...
int			rc_jtoc_get_default_walls(t_wall *walls, t_conf_json *conf,
		t_jnode *n)
{
	t_wall	cur_wall;
	int		cur_id;
	...
	while (n)
	{
		...
		if (get_default_wall_textures(&cur_wall, conf, n, cur_id))
			return (rc_jtoc_sdl_log_error(
						"GET TEXTURES FOR DEFAULT WALL FAILURE", cur_id));
