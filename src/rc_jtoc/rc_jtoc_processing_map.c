...
static int	check_bounds(int **map, int h_map, int w_map)
{
	...
	i = -1;
	while (++i < w_map)
		if (map[0][i] <= 0 || map[0][i] > 99 ||
			map[h_map - 1][i] <= 0 || map[h_map - 1][i] > 99)
			return (FUNCTION_FAILURE);
	i = -1;
	while (++i < h_map)
		if (map[i][0] <= 0 || map[i][0] > 99 ||
			map[i][w_map - 1] <= 0 || map[i][w_map - 1] > 99)
			return (FUNCTION_FAILURE);
	return (FUNCTION_SUCCESS);
}

static int	get_phys_map_helper(t_rc_main *m, int **map, int i, int j)
{
	if (map[i][j] < 0 || map[i][j] > 999 ||
		(map[i][j] > 99 && map[i][j] < 900))
		return (rc_jtoc_sdl_log_error("BAD ID", -1));
	if (map[i][j] == 999)
	{
		m->player.x = i + 0.5f;
		m->player.y = j + 0.5f;
...
static int	get_phys_map_and_objs_num(t_rc_main *m, int **map)
{
	...
	while (++i < m->map_h)
	{
		...
		while (++j < m->map_w)
		{
			if (map[i][j] == 999)
				player_id_flag++;
			if (get_phys_map_helper(m, map, i, j))
				return (FUNCTION_FAILURE);
		}
	}
	if (player_id_flag != 1)
